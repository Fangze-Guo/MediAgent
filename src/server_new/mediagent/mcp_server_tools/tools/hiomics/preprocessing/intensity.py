...
class WindowingTransformd(MapTransform):
    def __init__(self, keys, window_width, window_level):
        """
        ...
        self.trans = WindowingTransform(window_width, window_level)

    def __call__(self, data):
        return {k: self.trans(v) for k, v in data.items() if k in self.keys}
...
class RandHistogramShift(RandomizableTransform):
    ...
    def __call__(self, img: NdarrayOrTensor, randomize: bool = True) -> NdarrayOrTensor:
        ...
        if img_max - img_min < 1e-5: # cannot use equality here due to numerical errors
            warn(
...
class RandHistogramShiftD:
    def __init__(self, keys, num_control_points = 10, prob: float = 0.1):
        self.keys = ensure_tuple(keys)
        self.trans = RandHistogramShift(num_control_points=num_control_points, prob=prob)
