...
const previewFile = ref<OutputFileInfo | null>(null)
...
const columns = [
  {
    ...
    dataIndex: 'name',
    key: 'name',
    sorter: (a: OutputFileInfo, b: OutputFileInfo) => a.name.localeCompare(b.name),
  },
  {
    title: '类型',
    dataIndex: 'type',
  ...
  },
  {
    ...
    key: 'size',
    width: 120,
    sorter: (a: OutputFileInfo, b: OutputFileInfo) => a.size - b.size,
  },
  {
    title: '修改时间',
    ...
    key: 'modifiedTime',
    width: 180,
    sorter: (a: OutputFileInfo, b: OutputFileInfo) => new Date(a.modifiedTime).getTime() - new Date(b.modifiedTime).getTime(),
  },
  {
    title: '操作',
    dataIndex: 'actions',
...
// 检查是否为图片文件
const isImageFile = (file: OutputFileInfo | null) => {
...
const fetchFiles = async (path: string = '.') => {
  try {
    loading.value = true
    const response = await getOutputFiles(path)
...
// 获取输出文件下载URL
const getOutputFileDownloadUrl = (filePath: string): string => {
...
// 处理文件/目录点击
const handleItemClick = (item: OutputFileInfo) => {
  if (item.isDirectory) {
    // 进入目录
...
// 下载文件
const downloadFile = (file: OutputFileInfo) => {
  const downloadUrl = getOutputFileDownloadUrl(file.path)
...
// 预览文件
const previewFileHandler = (file: OutputFileInfo) => {
...
// 监听输出文件上传事件
const handleOutputFileUploadEvent = () => {
  openFileUploadDialog()
}
...
onMounted(() => {
  fetchFiles()
  // 监听刷新事件
  window.addEventListener('refresh-output-files', handleRefreshEvent)
  window.addEventListener('refresh-output-files-to-root', handleRefreshToRootEvent)
  window.addEventListener('open-output-file-upload', handleOutputFileUploadEvent)
})

// 组件卸载时移除事件监听
onUnmounted(() => {
  window.removeEventListener('refresh-output-files', handleRefreshEvent)
  window.removeEventListener('refresh-output-files-to-root', handleRefreshToRootEvent)
  window.removeEventListener('open-output-file-upload', handleOutputFileUploadEvent)
})
</script>
