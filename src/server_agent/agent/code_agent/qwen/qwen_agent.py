...
class QwenAgent(BaseCodeAgent):
    ...
    async def _execute(
        ...
        try:
            ...
            logger.info(f"[QwenAgent] 正在发送消息到 Qwen Code{session_info}{file_info}")

            # 使用会话管理器发送消息
            chunk_count = 0
            if session_id:
                logger.info(f"[QwenAgent] 正在恢复会话，UUID: {session_id}")
                async for chunk in self.session_manager.send_message(session_id, prompt, is_file=is_file, use_stream_json=use_stream_json):
                    chunk_count += 1
                    if chunk_count == 1:
                        logger.info(f"[QwenAgent] 已收到第一个数据块")
                    if chunk_count % 10 == 0:
                        logger.info(f"[QwenAgent] 已收到 {chunk_count} 个数据块")
                    yield chunk
            else:
                # 新会话
                logger.info(f"[QwenAgent] 正在创建新会话")
            ...
            logger.info(f"[QwenAgent] 消息发送完成，共 {chunk_count} 个数据块")

        except Exception as e:
            logger.error(f"[QwenAgent] 执行 Qwen Code 时发生错误: {e}")
            raise
    ...
    async def close_session(self, session_id: str):
        ...
        logger.info(f"Qwen 会话已关闭（状态已标记）: {session_id}")

    async def close_all_sessions(self):
        """关闭所有会话（仅标记状态，不删除Qwen的JSONL文件）"""
        # 注意：不再删除Qwen的JSONL文件，因为由审计服务管理会话状态
        logger.info("所有 Qwen 会话已关闭（状态已标记）")
